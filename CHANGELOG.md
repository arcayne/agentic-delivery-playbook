...
- Added PRD visibility guidance and templates: broad runs now keep a status dashboard plus PRD implementation ledger mapping requirements to slices, evidence, gaps, and next gates.
...
- Added handoff-economics guidance for read-only context/guardrail lanes and model-mix cost claims: keep large outputs separate, synthesize briefly, distinguish pre-implementation guardrails from final QA, and record telemetry instead of claiming savings.
